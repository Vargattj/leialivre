...
@section('title', 'Editar Citação')

@section('content')
<div class="mb-6">
    <h1 class="text-3xl font-bold text-gray-800">Editar Citação</h1>
</div>
...
<div class="bg-white shadow-md rounded-lg p-6">
    <form action="{{ route('admin.quotes.update', $quote) }}" method="POST">
        ...
        <div class="mb-4">
            ...
            <select name="book_id" id="book_id" 
                ...
                @foreach ($books as $book)
                    <option value="{{ $book->id }}" {{ (old('book_id', $quote->book_id) == $book->id) ? 'selected' : '' }}>
                        {{ $book->title }}
                    </option>
                @endforeach
            </select>
        ...
        <div class="mb-4">
            ...
            <select name="author_id" id="author_id" 
                ...
                @foreach ($authors as $author)
                    <option value="{{ $author->id }}" {{ (old('author_id', $quote->author_id) == $author->id) ? 'selected' : '' }}>
                        {{ $author->name }}
                    </option>
                @endforeach
            </select>
        ...
        <div class="mb-4">
            ...
            <input type="text" name="page_number" id="page_number" value="{{ old('page_number', $quote->page_number) }}"
        ...
        <div class="mb-4">
            ...
            <input type="number" name="order" id="order" value="{{ old('order', $quote->order) }}" min="0"
        ...
        <div class="mb-6">
            <label class="flex items-center">
                <input type="checkbox" name="is_active" value="1" {{ old('is_active', $quote->is_active) ? 'checked' : '' }}

...
<head>
    ...
    <meta name="google-adsense-account" content="ca-pub-9513325314205628">
    ...
    <noscript><link rel="stylesheet" href="https://fonts.googleapis.com/css2?family=Pacifico&display=swap"></noscript>
    {{-- Sitemap --}}
    ...
    <script>
        ...
        function trackDownload(bookId, bookTitle, format) {
            ...
            if (typeof mixpanel !== 'undefined') {
                mixpanel.track('book_download', payload);
            }
        }

        // Eventos do funil do guia gratuito (guide_offer_view, guide_email_submit, ...).
        // analytics_events é a fonte da verdade; gtag/dataLayer/mixpanel são a camada exploratória.
        function trackGuideEvent(eventType, metadata = {}) {
            const payload = { event_type: eventType, ...metadata };

            gtag('event', eventType, payload);

            window.dataLayer.push({ event: eventType, ...payload });
    ...
    mixpanel.init('{{ config("services.mixpanel.token") }}', {debug: {{ app()->environment('production') ? 'false' : 'true' }}, track_pageview: false, persistence: 'localStorage'});
    </script>
    <!-- End Mixpanel -->
    @endif
</head>
